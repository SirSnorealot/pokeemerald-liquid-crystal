...
static bool32 IsCeladonDeptStore(const struct MapHeader *mapHeader)
{
 ...
 || !defined(LAYOUT_CELADON_CITY_DEPARTMENT_STORE_2F) \
 || !defined(LAYOUT_CELADON_CITY_DEPARTMENT_STORE_3F) \
 || !defined(LAYOUT_CELADON_CITY_DEPARTMENT_STORE_4F) \
 || !defined(LAYOUT_CELADON_CITY_DEPARTMENT_STORE_5F) \
 || !defined(LAYOUT_CELADON_CITY_DEPARTMENT_STORE_ELEVATOR)
    // Some merge states omit Celadon map layouts from generated constants.
    // Keep this logic build-safe while still enabling it when all constants exist.
    return FALSE;
#else
...
#ifdef LAYOUT_CELADON_CITY_DEPARTMENT_STORE_ROOF
     && mapHeader->mapLayoutId != LAYOUT_CELADON_CITY_DEPARTMENT_STORE_ROOF
#endif
#ifdef LAYOUT_CELADON_CITY_DEPARTMENT_STORE_ROOFTOP
     && mapHeader->mapLayoutId != LAYOUT_CELADON_CITY_DEPARTMENT_STORE_ROOFTOP
#endif
