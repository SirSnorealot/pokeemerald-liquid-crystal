...
static void Task_Scene1_DittoAnimation(u8 taskId)
{
    ...
    // Original Emerald flow went to the camera pan scene here:
    // if (gIntroFrameCounter > TIMER_SPARKLES)
    // {
...
//==========================================================================
// Crystal intro (GBC -> GBA port)
//
// A port of the Pokémon Crystal opening cinematic. The GBC assets are
// converted to GBA-native data by dev_scripts/crystal_intro/convert.py:
//  - BG tiles are 4bpp with GBC pixel values 0..3 stored as 1..4, so that
//    index 0 stays transparent and the black backdrop letterboxes the
//    160x144 GBC scene inside the 240x160 GBA screen.
//  - Tilemaps carry the per-tile GBC palette in the GBA palette bits.
//  - Palette effects (Unown fades, letter fades) are recreated at runtime
//    and write to palette entries 1..4 of each 16-color slot.
//
// Scene order (as in Crystal):
//  Unown A -> panorama -> Unowns H/I -> panorama + Suicune runs by ->
//  Pichu & Wooper in the grass -> many Unown -> Suicune runs and jumps ->
//  Suicune leap (BG) -> Suicune close-up pan -> Suicune from behind with
//  Unowns appearing -> silhouette -> fade to white -> "CRYSTAL" in Unowns
//==========================================================================
...
// Colors loaded for the Unowns that appear around Suicune (gfx/intro/unown_1.pal)
...
// Fade one Unown in/out of the darkness. val counts 0..63 and mirrors at 31,
// producing the white/light-blue/blue shades of the Unown's body.
...
// Fills the per-scanline BG0 h-offsets to fake perspective: trees in the
// back scroll at half the speed of the grass in the front.
