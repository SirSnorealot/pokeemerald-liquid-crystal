...
void CableCarWarp(void)
{
    if (gSpecialVar_0x8004 != 0)
        SetWarpDestination(MAP_GROUP(MAP_ROUTE36_RUINS_OF_ALPH_GATE), MAP_NUM(MAP_ROUTE36_RUINS_OF_ALPH_GATE), WARP_ID_NONE, 6, 4);
    else
        SetWarpDestination(MAP_GROUP(MAP_ROUTE36_RUINS_OF_ALPH_GATE), MAP_NUM(MAP_ROUTE36_RUINS_OF_ALPH_GATE), WARP_ID_NONE, 6, 4);
}
...
u8 TryUpdateRusturfTunnelState(void)
{
    if (!FlagGet(FLAG_RUSTURF_TUNNEL_OPENED)
        && gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(MAP_SPROUT_TOWER_1F)
        && gSaveBlock1Ptr->location.mapNum == MAP_NUM(MAP_SPROUT_TOWER_1F))
    {
